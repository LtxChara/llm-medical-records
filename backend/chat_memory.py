...
class MedicalHistoryConfig:
    ...
    max_length: int = 10  # Increased default max length
    system_prompt: Optional[str] = None
    required: bool = False

class HistoryTypeConfigs:
    """Predefined configurations for standard history types."""
    # Ensure keys match the field names used elsewhere (e.g., VALID_FIELDS in server.py)
    主诉 = MedicalHistoryConfig(
        max_length=10, system_prompt=system_prompt_templates.get("主诉"), required=True
    )
    现病史 = MedicalHistoryConfig(
        max_length=15, system_prompt=system_prompt_templates.get("现病史"), required=True # Increased length for more detail
    )
    ...
    过敏史 = MedicalHistoryConfig(
        max_length=5, system_prompt=system_prompt_templates.get("过敏史"), required=False # Often shorter
    )
    诊断 = MedicalHistoryConfig(
        max_length=5, system_prompt=system_prompt_templates.get("诊断"), required=True
    )
    # Add other potential fields if needed, e.g., 体格检查, 辅助检查
...
class MedicalHistoryManager:
    ...
    def add_message(self, history_type: str, role: str, content: str):
        ...
        if not isinstance(content, str) or not content.strip():
            # Allow empty string for AI potentially, but log warning for user
            if role == "user":
                 ...
                 content = "" # Ensure it's an empty string, not None or whitespace
        ...
        if role == "user":
            ...
            history.add_ai_message(content.strip()) # Store AI response as well
    ...
    def get_user_all_input(self, history_type: str) -> str:
        """Concatenates all user inputs for a history type, potentially numbered."""
        if history_type not in self.histories:
            raise ValueError(f"Unknown history type: {history_type}")

        user_inputs = []
        # Removed numbering to simplify, just concatenate user messages.
        # LLM should handle context based on order.
        ...
        if not user_inputs:
            ...
            return "" # Return empty string if no user input yet

        return "\n".join(user_inputs) # Simple concatenation
    ...
    def to_openai_format(self, history_type: str) -> List[Dict[str, str]]:
        ...
        for msg in self.histories[history_type]["history"].messages:
            ...
            if isinstance(msg, SystemMessage):
                ...
                role = "assistant" # Correct role for AI messages in OpenAI API
    ...
    def save_to_json(self, file_path: str):
        ...
        try:
            ...
            for ht, data in self.histories.items():
                save_data[ht] = {
                    "config": asdict(data["config"]), # Convert dataclass to dict
                    "messages": [
    ...
    def load_from_json(cls, file_path: str):
        ...
        try:
            ...
            instance = cls(configs={}) # Start with empty configs
            for ht, ht_data in data.items():
                try:
                    # Recreate config object from loaded dict
                    ...
                    for msg_data in loaded_messages:
                        ...
                        if msg_type == "system":
                           # Avoid adding duplicate system prompt if already added by add_history_type
                    ...
                    if ht in instance.histories:
                        del instance.histories[ht] # Remove partially loaded/invalid history type
